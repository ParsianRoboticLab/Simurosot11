...
class CPerfTimerT
		: public CPerfTimer { // You only need to use types of this class if a timer is going to be shared between threads
public:
	...
	virtual ~CPerfTimerT() { CloseHandle(m_hMutex); }
...
inline void CPerfTimer::Init(BOOL bStart) {
	if (!m_Freq) { // Initialization should only run once
...
inline void CPerfTimer::Start(BOOL bReset) { // Start from current value or optionally from 0
	__int64 i;
...
inline void CPerfTimer::Stop() { // Stop timing. Use Start afterwards to continue
	Lock();
	if (m_Start <= 0) {
		Unlock();
...
inline BOOL CPerfTimer::IsRunning() { // Returns FALSE if stopped.
	Lock();
...
inline const double CPerfTimer::Elapsed() { // Returns elapsed time in seconds
	CPerfTimer Result(*this);
	Result.Stop();
	return (double) (-Result.m_Start) / (double) m_Freq;
}

inline const double CPerfTimer::Elapsedms() { // Returns elapsed time in milliseconds
	CPerfTimer Result(*this);
	Result.Stop();
	return (-Result.m_Start * 1000.0) / (double) m_Freq;
}

inline const double CPerfTimer::Elapsedus() { // Returns elapsed time in microseconds
	CPerfTimer Result(*this);
	Result.Stop();
